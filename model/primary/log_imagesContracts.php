...
/**
 * Class log_imagesContracts
 * @package model
 */
class log_imagesContracts extends log_images implements ModelContracts
{
    ...
    {
        $sql = "SELECT id FROM log_images WHERE dflag = 0 AND id = @1;";
    ...
    {
        $sql = sprintf("SELECT id FROM log_images WHERE dflag = 0 AND (%s = @1);", $column);
    ...
    {
        $sql = "SELECT COUNT(id) data FROM log_images WHERE dflag = 0;";
    ...
    public static function GetDataSizeWhere($condition = [])
    {
        ...
        $sql = sprintf("SELECT COUNT(id) data FROM log_images WHERE dflag = 0 %s;", $strings);
        $data = DBI::Prepare($sql)->FirstRow();
        return (int)$data['data'];
    }
    ...
    public static function SearchData($keyword = array())
    {
        ...
        $sql = sprintf("SELECT id, images_id, user_id, processing_time
        FROM log_images
