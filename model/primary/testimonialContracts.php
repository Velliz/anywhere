...
use Exception;
use plugins\model\primary\testimonial;
...
/**
 * Class testimonialContracts
 * @package model
 */
class testimonialContracts extends testimonial implements ModelContracts
{

    public static function GetData()
    {
        $sql = "SELECT id, user_id, signature, subject, testimonial, is_valid, validation_date
        FROM testimonial
        WHERE dflag = 0";
    ...
    {
        $sql = "SELECT id FROM testimonial WHERE dflag = 0 AND id = @1;";
    ...
    {
        $sql = sprintf("SELECT id FROM testimonial WHERE dflag = 0 AND (%s = @1);", $column);
    ...
    {
        $sql = "SELECT COUNT(id) data FROM testimonial WHERE dflag = 0;";
