...
use Exception;
use plugins\model\primary\status;
...
/**
 * Class statusContracts
 * @package model
 */
class statusContracts extends status implements ModelContracts
{

    public static function GetData()
    {
        $sql = "SELECT id, status, description, limitations
        FROM status
        WHERE dflag = 0";
    ...
    {
        $sql = "SELECT id FROM status WHERE dflag = 0 AND id = @1;";
    ...
    {
        $sql = sprintf("SELECT id FROM status WHERE dflag = 0 AND (%s = @1);", $column);
    ...
    {
        $sql = "SELECT COUNT(id) data FROM status WHERE dflag = 0;";
    ...
    public static function GetDataSizeWhere($condition = [])
    {
        ...
        $sql = sprintf("SELECT COUNT(id) data FROM status WHERE dflag = 0 %s;", $strings);
        $data = DBI::Prepare($sql)->FirstRow();
        return (int)$data['data'];
    }
    ...
    public static function SearchData($keyword = array())
    {
        ...
        $sql = sprintf("SELECT id, status, description, limitations
        FROM status
