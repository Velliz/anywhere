...
use Exception;
use plugins\model\primary\constanta;
...
/**
 * Class constantaContracts
 * @package model
 */
class constantaContracts extends constanta implements ModelContracts
{
    ...
    {
        $sql = "SELECT id FROM constanta WHERE dflag = 0 AND id = @1;";
    ...
    {
        $sql = sprintf("SELECT id FROM constanta WHERE dflag = 0 AND (%s = @1);", $column);
    ...
    {
        $sql = "SELECT COUNT(id) data FROM constanta WHERE dflag = 0;";
    ...
    public static function GetDataSizeWhere($condition = [])
    {
        ...
        $sql = sprintf("SELECT COUNT(id) data FROM constanta WHERE dflag = 0 %s;", $strings);
        $data = DBI::Prepare($sql)->FirstRow();
        return (int)$data['data'];
    }
